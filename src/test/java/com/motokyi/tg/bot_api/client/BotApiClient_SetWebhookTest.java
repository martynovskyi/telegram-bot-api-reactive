...
public class BotApiClient_SetWebhookTest extends BotClientTest {
    ...
    public void successful() {
        ...
        stubFor(post(urlEqualTo(ApiUrls.SET_WEBHOOK))
                .withRequestBody(matchingJsonPath(jsonPath(ApiProperties.URL), equalTo(URL))
                        .and(matchingJsonPath(jsonPath(ApiProperties.IP_ADDRESS), equalTo(setWebhook.getIpAddress())))
                        .and(matchingJsonPath(jsonPath(ApiProperties.MAX_CONNECTIONS), equalTo(setWebhook.getMaxConnections().toString())))
                        .and(matchingJsonPath(jsonPath(ApiProperties.ALLOWED_UPDATES), equalToJson("[\"message\"]")))
                        .and(matchingJsonPath(jsonPath(ApiProperties.SECRET_TOKEN), equalTo(setWebhook.getSecretToken())))
                )
